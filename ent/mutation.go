...
func (m *PinMutation) OldOrigins(ctx context.Context) (v []string, err error) {
	if !m.op.Is(OpUpdateOne) {
		return v, errors.New("OldOrigins is only allowed on UpdateOne operations")
	}
	if m.id == nil || m.oldValue == nil {
		return v, errors.New("OldOrigins requires an ID field in the mutation")
	}
	oldValue, err := m.oldValue(ctx)
	if err != nil {
...
// AppendOrigins adds s to the "Origins" field.
func (m *PinMutation) AppendOrigins(s []string) {
	m.append_Origins = append(m.append_Origins, s...)
}

// AppendedOrigins returns the list of values that were appended to the "Origins" field in this mutation.
func (m *PinMutation) AppendedOrigins() ([]string, bool) {
	if len(m.append_Origins) == 0 {
		return nil, false
	}
	return m.append_Origins, true
}
