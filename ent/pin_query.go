...
func (pq *PinQuery) GroupBy(field string, fields ...string) *PinGroupBy {
	grbuild := &PinGroupBy{config: pq.config}
...
func (pq *PinQuery) Select(fields ...string) *PinSelect {
	...
	selbuild := &PinSelect{PinQuery: pq}
	selbuild.label = pin.Label
	selbuild.flds, selbuild.scan = &pq.fields, selbuild.Scan
	return selbuild
}

// Aggregate returns a PinSelect configured with the given aggregations.
func (pq *PinQuery) Aggregate(fns ...AggregateFunc) *PinSelect {
...
func (pq *PinQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Pin, error) {
	var (
		nodes       = []*Pin{}
		withFKs     = pq.withFKs
		_spec       = pq.querySpec()
	...
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Pin).scanValues(nil, columns)
	}
	...
	if query := pq.withUser; query != nil {
		if err := pq.loadUser(ctx, query, nodes, nil,
			func(n *Pin, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PinQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Pin, init func(*Pin), assign func(*Pin, *User)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_pins" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pgb *PinGroupBy) Scan(ctx context.Context, v any) error {
...
func (pgb *PinGroupBy) sqlScan(ctx context.Context, v any) error {
...
func (ps *PinSelect) Aggregate(fns ...AggregateFunc) *PinSelect {
	ps.fns = append(ps.fns, fns...)
	return ps
}

// Scan applies the selector query and scans the result into the given value.
func (ps *PinSelect) Scan(ctx context.Context, v any) error {
...
func (ps *PinSelect) sqlScan(ctx context.Context, v any) error {
