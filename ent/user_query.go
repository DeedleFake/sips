...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withPins; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_pins
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_pins" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_pins" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Pins = append(node.Edges.Pins, n)
		}
	}
