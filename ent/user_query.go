...
func (uq *UserQuery) QueryPins() *PinQuery {
	query := &PinQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithPins(opts ...func(*PinQuery)) *UserQuery {
	query := &PinQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPins; query != nil {
		if err := uq.loadPins(ctx, query, nodes,
			func(n *User) { n.Edges.Pins = []*Pin{} },
			func(n *User, e *Pin) { n.Edges.Pins = append(n.Edges.Pins, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadPins(ctx context.Context, query *PinQuery, nodes []*User, init func(*User), assign func(*User, *Pin)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Pin(func(s *sql.Selector) {
		s.Where(sql.InValues(user.PinsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_pins
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_pins" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_pins" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
